  ...
  return <div className="page dossier">
    ...
    <section className="panel dossier-context">
      ...
      <dl className="context-grid">
        <div><dt>Seller</dt><dd>{context?.seller ?? "—"}</dd></div><div><dt>Operação</dt><dd>{context?.operation ?? "—"}</dd></div><div><dt>Transportadora</dt><dd>{context?.carrier ?? "—"}</dd></div><div><dt>Período</dt><dd>{context?.period ?? "—"}</dd></div>
    ...
    <section className="panel dossier-results">
      ...
      {visible.length ? <div className="table-wrap"><table className="dossier-table"><thead><tr><th>Status</th><th>Rastreio / pedido</th><th>Marketplace</th><th>SKU(s)</th><th>Regra</th><th className="number">Cobrado</th><th className="number">Esperado</th><th className="number">Recuperável</th><th></th></tr></thead>
      <tbody>{visible.map(item => <tr key={item.finding_id} className="clickable-row" onClick={() => setSelected(item)} tabIndex={0} onKeyDown={event => event.key === "Enter" && setSelected(item)}><td><StatusBadge status={item.status} /></td><td><b>{item.tracking_number ?? item.shipment_id ?? "—"}</b><small>{item.order_id ?? item.pack_id ?? "Sem pedido"}</small></td><td><span className="marketplace-pill">{item.marketplace ?? "Não informado"}</span><small>{item.carrier ?? "—"}</small></td><td className="sku-cell"><b>{findingSkuDisplay(item)}</b><small>{item.quantity != null ? `${item.quantity} unidade(s)` : "Quantidade não informada"}</small></td><td><b>{item.rule_id ?? "Sem regra"}</b><small>{item.match_method ?? "Sem conciliação"}</small></td><td className="number">{formatCurrency(item.charged_amount)}</td><td className="number">{formatCurrency(item.expected_amount)}</td><td className="number recover">{formatCurrency(item.recoverable_amount)}</td><td><span className="row-open">Abrir</span></td></tr>)}</tbody></table></div> : <EmptyState />}
